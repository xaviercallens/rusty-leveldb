...
/// Memory-mapped file for zero-copy random access reads.
/// Falls back to pread for files smaller than the mmap threshold.
pub struct MmapRandomAccess {
    mmap: Mmap,
}

impl MmapRandomAccess {
    /// Create a new mmap-backed random access file.
    /// Safety: The file must not be modified while the mmap is active.
    /// LevelDB SSTable files are immutable after creation, so this is safe.
    pub fn new(file: &File) -> io::Result<MmapRandomAccess> {
        // SAFETY: SSTable files are immutable once written — no concurrent modification.
...
impl RandomAccess for MmapRandomAccess {
    fn read_at(&self, off: usize, dst: &mut [u8]) -> Result<usize> {
        ...
        if end > self.mmap.len() {
            let avail = if off < self.mmap.len() {
                self.mmap.len() - off
            } else {
                return Ok(0);
            };
            dst[..avail].copy_from_slice(&self.mmap[off..off + avail]);
            Ok(avail)
        } else {
...
/// Minimum file size to use mmap (16 KB). Smaller files use pread.
...
impl Env for PosixDiskEnv {
    ...
    fn open_random_access_file(&self, p: &Path) -> Result<Box<dyn RandomAccess>> {
        ...
        // Use mmap for files above threshold (SSTable files are immutable)
        let meta = file
            .metadata()
            .map_err(|e| map_err_with_name("metadata (randomaccess)", p, e))?;

        if meta.len() >= MMAP_THRESHOLD {
            match MmapRandomAccess::new(&file) {
                Ok(mmap) => return Ok(Box::new(mmap)),
                Err(_) => {} // fall through to pread
            }
        }
